...
const getFallbackFeedback = () => ({
  ...
  summary: "Great job completing the interview. (Note: This is an automated fallback report because the AI service is currently busy. Your answers were recorded successfully.)"
});
...
exports.chat = async (req, res) => {
  try {
    ...
    const aiMessageCount = interview.conversation.filter(m => m.role === 'ai').length;

    if (aiMessageCount >= 5) {
      const feedbackJson = await getFinalFeedback(interview.conversation, interview.jobDescription);
    ...
    const nextQuestion = await getAIResponse(interview.conversation, interview.jobDescription);
