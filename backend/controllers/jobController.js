...
const calculateAIScore = async (resumePath, jobDescription) => {
  let resumeText = "";
  try {
    // FIX 1: Use 'resumePath' (the function parameter), NOT 'resumeUrl'
    ...
    const prompt = `
      You are an ATS. 
      Analyze the Resume for the following Job Description (JD).
      JD: "${jobDescription.substring(0, 500)}"
      Resume: "${resumeText.substring(0, 5000)}" 
      Task: Rate the match from 0 to 100. Return ONLY the integer score.
    `;
    ...
    // This logs why it failed so you can troubleshoot further
  ...
  try {
    ...
    for (let app of job.applicants) {
      ...
      const needsCalculation =
        !app.matchScore || app.matchScore === 0 || app.isFallbackScore === true;

      if (!needsCalculation) {
        // CASE A: VALID SCORE EXISTS
        console.log(
          `[CACHE HIT] ${candidateName} already has AI Score: ${app.matchScore}. Skipping.`,
        );
      } else {
        // CASE B: CALCULATION NEEDED
        const reason = app.isFallbackScore
          ? "Retrying Fallback Score"
          : "New Applicant";
        ...
        if (app.resumeUrl) {
          ...
          if (
            ...
            console.log(
              `   💾 UPDATING DB: Score ${app.matchScore} -> ${result.score}`,
            );
            app.matchScore = result.score;
            app.isFallbackScore = result.isFallback;
            updated = true;
